...
#define putchar(c)      do { while (UART0_STATE & 1U); UART0_DATA = (c); } while(0)
...
static int tiny_print( char **out, const char *format, va_list args, unsigned int buflen )
{
	...
	for (; *format != 0; ++format) {
		if (*format == '%') {
			...
			if( *format == 's' ) {
				/* NOTE: inherited from FreeRTOS demo. va_arg(args, int) for a
				 * pointer is technically UB, but safe on Cortex-M3 where
...
/* NOTE: No buffer bounds checking — prefer snprintf for new callers */
